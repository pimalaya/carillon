//! Secret prompts shared by the backend wizards.
//!
...
//! a raw value. A known provider or broker yields an argv command (a
//! TOML array); a custom command is a shell string. carillon only
//! *reads* the secret: the value must already be stored, and a missing
//! one surfaces when the connection is tested right after.
...
/// pre-existing secret is read exactly as named.
...
/// (typically the carillon account name).
