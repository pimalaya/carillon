...
//! OAuth 2.0 brokers (Ortie, pizauth, oama), both also a custom command
//! or a raw value. A known provider yields an argv command, a TOML array;
//! a custom command is a shell string.
//!
...
///
/// It combines the OS keyrings, for a token generated on the provider,
/// with the OAuth 2.0 brokers when `oauth` is true, a broker printing a
/// fresh token on every read. `key_default` seeds the entry or handle.
...
/// Builds a [`Secret::Command`] from an argv, the shell-less form a known
/// keyring provider or token broker yields, serialized as a TOML array.
...
/// fallback, serialized as a TOML string.
